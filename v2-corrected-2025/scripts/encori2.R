...
rna_rna <- function(assembly="hg38",
  ...
  if (!requireNamespace("readr", quietly = TRUE)) stop("Please install the 'readr' package.")
    ...
    for (rna in RNA){
      for (cell in cellType){
        ...
        link <- paste("https://rnasysu.com/encori/api/RNARNA/?",
                      ...
                      "&expNum=",expNum,
                      "&cellType=",cell, sep = "")
        response <- try(httr::GET(link, httr::timeout(120)), silent = TRUE)
        
        if (inherits(response, "try-error") || httr::status_code(response) != 200) {
          message("No se pudo obtener respuesta para: ", rna)
          next
        }
        ...
        if (length(lines) <= 1 || grepl("The RNA parameter", lines[1])) {
          next
        }
